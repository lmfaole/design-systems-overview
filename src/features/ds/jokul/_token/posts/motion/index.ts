...
import { createMotionPreview } from "@/features/ds/jokul/_token/components/MotionPreview";
    ...
    tokenOverview: [
        {
            ...
            rows: timingTokens.map(({ path, token, value, usage }) => [
                createTableExampleFrame(
                    ...
                    createMotionPreview({
                        timing: token,
                        easing: "--jkl-motion-easing-entrance",
                    }),
                ),
                code(formatPublicMotionTokenPath(path)),
                code(token),
                text(value),
                text(usage),
            ]),
        },
        {
            ...
            rows: easingTokens.map(({ path, token, curve, usage }) => [
                createTableExampleFrame(
                    "motion",
                    createMotionPreview({
                        timing: "--jkl-motion-timing-expressive",
                        easing: token,
                    }),
                ),
                code(formatPublicMotionTokenPath(path)),
