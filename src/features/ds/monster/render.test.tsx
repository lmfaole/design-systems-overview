...
describe("monster render helpers", () => {
    ...
    it("serializes further reading descriptions when present", () => {
        const post = getPatternPost("lastetilstander");
        ...
        expect(describedResource?.descriptionHtml).toContain("ventestatus");
    });
    ...
    it("rewrites aria and hash references to the namespaced live example ids", () => {
        ...
        expect(Array.from(ids).every((id) => id.startsWith("monster-example-"))).toBe(true);
    });
});
